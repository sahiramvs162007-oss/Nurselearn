...
exports.getLogin = (req, res) => {
  if (req.session && req.session.userId) {
    req.flash(
      "error",
      "Ya tienes una sesión abierta. Cierra sesión para cambiar de perfil.",
    );
    const redirectMap = {
      admin: "/admin",
    ...
    return res.redirect(redirectMap[req.session.userRol] || "/");
  }
  ...
  try {
    ...
    if (user.rol === "instructor") {
      const ficha = await Ficha.findOne({
        "instructores.instructor": user._id,
      });
      if (!ficha) {
        req.flash(
          "error",
          "No tienes fichas asignadas. Contacta al administrador.",
        );
        return res.redirect("/auth/login");
      }
    }
